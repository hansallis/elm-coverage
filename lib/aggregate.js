...
module.exports = function(sourcePath, exclude) {
    return Promise.all([fs.readdir(elmTestGeneratedDir), readInfo()])
        ...
        .then(function(unfilteredCoverageData) {
            var moduleMap = {};

            const coverageData = Object.fromEntries(Object.entries(unfilteredCoverageData).filter(([key, val]) => !exclude.test(key)));
